...
try {
    if (isset ($_POST['d_register'])) {
        ...
        if ($count == 0) {
            ...
            if ($conn->query($query)) {
                redirect('admin_dashboard.php','Registration Successfull');
            } else {
                onlyAlertMessage('An error occured while Registering Donor, Please Try Again!');
            }
        } else {
            onlyAlertMessage('Error: Email Id already Exists, Please Try Again with different Email Id!');
        }
    }
    ...
    <body>
        ...
        <main>
            ...
            <form id="donor_form" action="" method="post" onsubmit="return validateform()">
                ...
                <input type="text" id="d_name" name="d_name" required value="<?php if (isset($_POST['d_name'])){echo ($_POST['d_name']);} ?>" placeholder='Prasanna Sonawane'><span id="name_error_logo"><img src="assets/images/cross.png"></span>
                <div id="name_err" class="error"></div>  

                <label for="d_age">Age :</label>
                <input type="number" id="d_age" name="d_age" required value="<?php if (isset($_POST['d_age'])){echo ($_POST['d_age']);} ?>" placeholder='25' ><span id="age_error_logo">
        ...
        <div id="age_err" class="error"></div>  
                ...
                <input text="text" id="d_address" name="d_address" value="<?php if (isset($_POST['d_address'])){echo ($_POST['d_address']);} ?>" placeholder='Pune,India'></input>
                <div class="error"></div> 
